...
export const fetchWeatherData = createAsyncThunk(
  ...
  async (cityName, { rejectWithValue }) => {
    try {
      const weatherResponse = await axios.get(`data/2.5/weather?`, {
        params: {
          q: cityName,
          units: "metric",
          lang: "tr",
...
export const updateCityData = (cityName, dt) => async (dispatch) => {
  try {
    const twoHoursInSeconds = 5 * 30 * 60;
    const nowInSeconds = Math.floor(Date.now() / 1000);
    if (nowInSeconds - dt > twoHoursInSeconds) {
      const updatedData = await dispatch(fetchWeatherData(cityName));
      if (fetchWeatherData.fulfilled.match(updatedData)) {
        // alert(`${cityName} Verileri başarıyla güncellendi`);
      }
    }
  } catch (error) {
